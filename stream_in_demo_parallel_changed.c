...
void show_image(){
	    ...
	    for(int row = 0; row < row_num; row++){
            for(int col = 0; col < col_num; col++){
                if(col - dispMap_left_copied.at<uint8_t>(row, col) < col_num && col - dispMap_left_copied.at<uint8_t>(row, col) >= 0){
                    consistency.at<uint8_t>(row, col) = (uint8_t)abs((dispMap_right_copied.at<uint8_t>(row,col - (int)(dispMap_left_copied.at<uint8_t>(row,col) >> 0)) - dispMap_left_copied.at<uint8_t>(row,col)));
                }
                else{
                    consistency.at<uint8_t>(row, col) = 255;
                }
                if(consistency.at<uint8_t>(row, col) < LR_threshold){
                    if(dispMap_copy.at<uint8_t>(row, col) == 127) {
                        dispMap_LRcheck_raw.at<uint8_t>(row, col) = (uint8_t)127;
                    }
                    dispMap_LRcheck_raw.at<uint8_t>(row, col) = dispMap_copy.at<uint8_t>(row, col);
                }
                else{
	    ...
	    medianBlur(dispMap_LR_check_scaled, dispMap_medianfilter, 7);
