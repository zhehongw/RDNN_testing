...
void write_vectr_file_start(){
    ...
    to_buffer(0b11001110000000000000000000001100);//"SD5
    //for PLL
...
void write_vector_file_stream(Mat &left_image, Mat &right_image, int frameCounter, int row_counter, int col_counter, bool even, int& result_mem_even, int& image_mem_even){
    ...
    if(((row_counter == 0 && col_counter == 0) || (row_counter == 0 && col_counter == 1)) && frameCounter == 0){
       // cout << "first" << endl;
        ...
        if(row_counter == 0 && col_counter == 2 && (frameCounter % 10 == 0)) { //transfer one outstanding packet
            ...
            to_buffer(control_result); 
        } else if(row_counter == 0 && col_counter == 2 && (frameCounter % 10 != 0)) {
            uint32_t control_result = 0;
...
int main () {
    ...
    */
    ...
    for(imageCounter = 0; imageCounter < 50; imageCounter++){
        ...
        cap_input.image_num = image_num;
        cap_input.imageSize = imageSize;
        //cap_input.validRoi = validRoi;
        //cap_input.rmap = rmap;
        cap_input.leftImage = left_cap;
        cap_input.rightImage = right_cap;
        ...
        pthread_mutex_lock(&transfer_lock);
        //cout << "preparing data" << endl;
        memcpy ( transfer_buffer, buffer, 1024 * sizeof(unsigned char) );
        transfer_buffer_prepared = true;
        //cout << "data prepared" << endl;
        //unlock for the transfer
        pthread_cond_signal(&transfer_cond);
        ...
        */
        ...
        CapImage(image_num, imageSize, validRoi, rmap, left, right);
        //CapImage(image_num, imageSize, validRoi, rmap, left_image, right_image);
        
        //imwrite("rectified_left.png", left);
        //imwrite("rectified_right.png", right);
        Rect region(x, y, width, height);

        //left_image = imread("left_rect_15.png", CV_LOAD_IMAGE_ANYDEPTH);
        //right_image = imread("right_rect_15.png", CV_LOAD_IMAGE_ANYDEPTH);

        Mat left_image(left, region);
        Mat right_image(right, region);
        ...
        //if(left_image.depth() == CV_8U) cout << "CV_8U" << endl;
        
        //left_image = imread("left.png", CV_LOAD_IMAGE_GRAYSCALE);
        //right_image = imread("right.png", CV_LOAD_IMAGE_GRAYSCALE);
